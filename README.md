...
A [Herdr](https://herdr.dev) plugin that publishes each pane's agent kind as a
`$kind` sidebar token, so you can show an agent's name *and* what it is.
...
This plugin publishes the kind as a separate `$kind` token, so the two never
...
Then use `$kind` in your agent rows, alongside `agent` for the name:
...
kind=<kind>`. No compiled binary, no polling, no daemon.
...
- When an agent exits and its pane returns to a shell, the pane keeps a stale
  `kind` token. It is never rendered, because the agents sidebar only lists
